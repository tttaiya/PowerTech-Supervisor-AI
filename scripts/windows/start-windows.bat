...
if not errorlevel 1 (
    ...
    if errorlevel 1 (
        echo [错误] MySQL 启动失败，请确保 Docker Desktop 已启动
        pause
        exit /b 1
    )
    echo [信息] 等待 MySQL 启动（15秒）...
...
if errorlevel 1 (
    ...
    REM 调用 API 上传 knowledge-docs 文档到向量数据库
    echo [9/9] 上传文档到向量数据库...
    for %%f in (knowledge-docs\*.md) do (
        echo   上传: %%~nxf
