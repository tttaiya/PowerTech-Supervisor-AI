...
class AIOpsService:
    ...
    async def execute(
        ...
        trace = trace_service.create_trace(session_id=session_id, user_input=user_input)
        ...
        try:
            ...
            async for event in self.graph.astream(
                ...
                for node_name, node_output in event.items():
                    ...
                    trace_service.record_node(
                        ...
                        output_snapshot=node_output,
                    )

                    # 根据节点类型生成不同的事件
                    if node_name == NODE_PLANNER:
                        if node_output.get("plan"):
                            trace.planner_output = node_output.get("plan", [])
                        ...
                        trace.replanner_decisions.append(
                            {
                                "action": node_output.get("replanner_action", "unknown"),
                                "remaining_plan": node_output.get("plan", []),
                                "has_response": bool(node_output.get("response")),
                            }
                        )
                        trace_service.save_trace(trace)
                    ...
                    formatted_event["trace_id"] = trace.trace_id
            ...
            trace_service.finish_trace(trace, final_response)
            ...
            trace_service.fail_trace(trace, str(e))
            yield {
                "type": "error",
                "stage": "error",
                "trace_id": trace.trace_id,
