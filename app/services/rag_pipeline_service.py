...
class RagPipelineService:
    ...
    def _default_vector_search(self, question: str, top_k: int, knowledge_base_ids: list[str] | None = None) -> list[Any]:
        ...
        try:
            rows = vector_search_service.search_similar_documents(question, limit, knowledge_base_ids=knowledge_base_ids)
            if not rows and self.db is not None:
                return self._db_chunk_search(question, top_k, knowledge_base_ids)
            return rows
        except Exception:
            if self.db is None:
                return []
            ...
            citations.append(
                {
                    ...
                    "download_url": f"/api/knowledge/documents/{chunk.document_id}/download" if chunk.document_id else None,
                }
            )
        return citations
