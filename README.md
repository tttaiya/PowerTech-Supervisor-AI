...
- 🔎 **Trace 可观测** - 记录诊断 trace_id、节点输出、工具调用、重规划决策和最终状态
- 🧰 **工具治理** - 统一工具注册、启停、超时、错误分类、原始输出留存和压缩摘要
- 🧠 **上下文压缩** - 对长日志和长工具结果抽取关键错误、异常指标、服务名和可能原因
- 📝 **Bad Case 复盘** - 支持失败案例创建、查询和沉淀，形成持续优化闭环
- 🖼️ **告警截图入口** - 支持从 OCR 文本解析告警字段并生成 AIOps 诊断任务
- 📊 **评测集** - 覆盖 RAG、AIOps、异常和截图场景，支持回归验收
- 🌐 **Web 界面** - 现代化 UI，支持多种对话模式：快速问答/流式对话
- 🔌 **MCP 集成** - 日志查询和监控数据工具接入

## 🚀 Agent OnCall 10 天差异化改造

本项目已按智能 OnCall 优化计划完成 Day01-Day10 的工程化增强：

| 天数 | 改造内容 | 目的 | 关键文件 |
|------|----------|------|----------|
| Day01 | 本地部署与环境检查 | 建立可运行基线 | `README.md` |
| Day02 | Trace 主链路 | 每次诊断可追踪 | `app/services/trace_service.py` |
| Day03 | 节点级 Trace | 看清 planner/executor/replanner 中间过程 | `app/services/aiops_service.py` |
| Day04 | 工具注册表 | 统一工具元数据和启停治理 | `app/agent/tool_registry.py` |
| Day05 | 工具执行器 | 统一超时、错误分类、原始输出和 trace 记录 | `app/agent/tool_executor.py` |
| Day06 | 上下文压缩 | 降低长日志噪声，保留结构化证据 | `app/services/context_compressor.py` |
| Day07 | Bad Case 复盘 | 失败样例沉淀和持续优化 | `app/services/bad_case_service.py` |
| Day08 | 告警截图解析 | 支持真实 OnCall 截图入口 | `app/services/screenshot_alert_parser.py` |
| Day09 | Demo 脚本 | 面试和演示有清晰主线 | `docs/demo_script.md` |
| Day10 | 评测集 | 用固定样例证明优化效果 | `eval/agent_eval_cases.json` |

详细讲解文档：

- `docs/agent_upgrade_summary.md`
- `docs/interview_talk_track.md`
- `docs/resume_bullets.md`
- `docs/context_compression_design.md`
- `docs/bad_case_review_template.md`
- `docs/multimodal_alert_design.md`
- `eval/run_eval_manual.md`
...
| 告警截图解析 | POST | `/api/aiops/screenshot` | 上传截图并生成结构化诊断任务 |
| Bad Case 创建 | POST | `/api/badcases` | 新增失败案例复盘 |
| Bad Case 列表 | GET | `/api/badcases` | 查询失败案例 |
