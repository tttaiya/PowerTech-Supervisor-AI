...
# App
DATABASE_URL=mysql+pymysql://biz_agent:biz_agent_123456@127.0.0.1:3307/super_biz_agent?charset=utf8mb4
...
```powershell
docker compose -f mysql-database.yml up -d
```
...
如果你当前还有历史 `super_biz_agent.db` 数据，可执行：

```powershell
.\.venv\Scripts\python.exe scripts\migrate_sqlite_to_mysql.py `
  --source-url sqlite:///./super_biz_agent.db `
  --target-url "mysql+pymysql://biz_agent:biz_agent_123456@127.0.0.1:3307/super_biz_agent?charset=utf8mb4" `
  --truncate-target
```

迁移完成后，将 `.env` 中的 `DATABASE_URL` 指向 MySQL，再重新启动服务。
...
- `traces/`
- `htmlcov/`
- `super_biz_agent.db`（仅历史 SQLite 迁移源）
